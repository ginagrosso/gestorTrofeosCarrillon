...
const CLIENTE_SEARCH_FIELDS: (keyof Cliente)[] = ['nombre', 'cuit', 'telefono', 'celular', 'email']
          ...
          <TableBody>
            {paginatedItems.map((cliente) => (
              <TableRow key={cliente.id}>
                ...
                <TableCell>{cliente.celular || cliente.telefono || '—'}</TableCell>
                <TableCell className="text-right">
                  <div className="flex justify-end gap-2">
                    {(cliente.celular || cliente.telefono) && (
                      <Button variant="ghost" size="icon" asChild>
                        <a href={getWhatsAppUrl((cliente.celular || cliente.telefono)!)} target="_blank" rel="noopener noreferrer">
